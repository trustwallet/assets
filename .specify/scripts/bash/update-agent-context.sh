...
# Remove flags from AGENT_TYPE if it was set to a flag
if [[ "$AGENT_TYPE" == "--skip-claude" ]]; then
    AGENT_TYPE=""
fi
    ...
    case "$agent_type" in
        claude)
            if [[ "$SKIP_CLAUDE" == true ]]; then
                log_info "Skipping CLAUDE.md update (--skip-claude flag set)"
            else
...
update_all_existing_agents() {
    ...
    # If no agent files exist, create a default Claude file (unless --skip-claude)
    if [[ "$found_agent" == false ]]; then
        if [[ "$SKIP_CLAUDE" == true ]]; then
            log_info "No existing agent files found, but --skip-claude flag set. Skipping."
        else
