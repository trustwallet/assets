...
- **Checklist:** NEVER pre-check any items — always leave them as `- [ ]`. Conditionally include items based on the `git diff --stat` output:
  ...
  - "Screenshots provided 📸 (required for UI changes)" — only if the PR touches UI files (files matching patterns like `*Screen.swift`, `*View.swift`, `TDSKit/Sources/**`, `design-system/src/**`, `feature/*/ui/src/**`, `android/app/src/main/res/**`, `*Screen.kt`, `react-native/packages/ui/src/**`, `react-native/packages/features/*/src/screens/**`). CI enforces this item is checked for these PRs. **Omit this item entirely** when no UI files are changed.
