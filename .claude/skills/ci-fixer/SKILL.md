...
|---|---|---|
| **Lint (TSDK/Android)** | `tsdk-run-checks`, `android-ktlint` | `ktlint`, formatting violations |
| **Lint (iOS)** | iOS lint/swiftlint jobs | `swiftlint`, lint violations |
| **Lint (React Native)** | `react-native-test` | `eslint`, `prettier`, lint errors |
...
| **Danger** | `danger` | Dependency plugin, module naming, Danger rules |
| **Test Failures** | Platform test jobs | `FAILED`, `XCTAssert`, `assertEquals`, test errors |
| **Build Failures** | Platform build jobs | Compilation errors, unresolved symbols, type mismatches |
...
|---|------------|----------|--------|-------------|
| 1 | android-ktlint | Lint | Failed | Auto-fix with ktlintFormat |
...
Run the appropriate formatter based on which lint check failed:

| Platform | Command |
|---|---|
| TSDK | `cd tsdk && ./gradlew ktlintFormatAll` |
| Android | `cd android && ./gradlew ktlintFormat` |
| iOS | `cd ios && make lint-fix` |
| React Native | `cd react-native && pnpm run validate:fix` |

After running, verify the formatter produced changes with `git diff --stat`.

#### 8d. Danger Violations

Read the Danger log output to identify specific violations:
- **Dependency plugin issues** — fix imports, module declarations
- **Module naming violations** — rename according to conventions
- Apply manual code edits based on the Danger output
...
- Lint formatters and RN validation are safe to run locally
- Do NOT run full platform builds locally (too slow, may need CI secrets)
