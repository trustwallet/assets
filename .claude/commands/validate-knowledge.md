...
2. **Discover repo context** — scan for `.claude/` files, `knowledge/` sections, `constitution.md`, `CLAUDE.md` routing instructions. Note which platforms, sections, and topics exist.

3. **Generate validation prompts** — create 10-15 questions covering:
   - Architecture patterns per platform (iOS, Android, TSDK, React Native — whichever exist)
   - Forbidden patterns / anti-patterns
   - Testing frameworks and conventions
   - Pre-commit / formatting requirements
   - Platform routing ("which guidelines for /ios/?")
...
4. **Answer each question yourself** — use Read, Glob, Grep to find the answer in the repo. For each answer, score 1-5:
   ...
   - 2 = Found something vague, missing platform context
...
| 2 | ... | 5 | ios/.claude/claude-architecture.md |
...
...
- Be honest about scores — a 3 is fine if the answer exists but lacks platform specificity
