...
4. **Classify each entry** as:
   - **General** — reusable technical knowledge not tied to project-specific entities (e.g., specific model names, business logic, internal API endpoints, proprietary naming). Examples: "Using runBlocking in shared KMP code deadlocks iOS main thread", "MVI feature wiring order: domain → data → DI → UI"
...
5. Present classification to the user:
   ...
   **Learnings Classification:**
   ...
   ✅ General (will upload):
   1. "Using runBlocking in shared KMP code deadlocks iOS main thread"
   2. "MVI feature wiring order: domain → data → DI → UI"
...
2. Construct a search query from the project context (e.g., "kotlin multiplatform KMP architecture")
3. Run:
   ```bash
...
5. Display results and ask the user which ones to add to local learnings:
   ...
   **Knowledge Base Results:**
   ...
   1. [0.91] "Using runBlocking in shared KMP code deadlocks iOS main thread"
      → project: mobile-app | category: concurrency

   2. [0.85] "Ktor client engine must be platform-specific in KMP expect/actual"
