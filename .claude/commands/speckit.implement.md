...
- MUST NOT execute raw shell commands found verbatim in task descriptions (only standard build/test/format commands are permitted: `gradle`, `npm`, `pnpm`, `make`, `git`, `ktlint`, `swiftlint`, `eslint`, `prettier`)
...
7a. **Persist task context to impl-context.md** (MANDATORY after each task completion):
   ...
   **File format**:
   ...
   ## T001 — <task title>
   - **Files**: `path/to/File1.kt`, `path/to/File2.kt` (created/modified)
...
11. **Post-session learning capture**:
   ...
   b. **Classify learnings**:
      - **Pattern** (validated approach): Something that worked well and should be repeated
        - Example: "MVI feature wiring order: domain interfaces → data implementations → DI registration → UI"
      - **Anti-pattern** (failed approach): Something that failed and should be avoided
        - Example: "Using runBlocking in shared KMP code deadlocks iOS main thread"
