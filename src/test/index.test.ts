...
describe('Check repository root dir', () => {
    ...
    test('Root should contains only predefined files', () => {
        dirActualFiles.forEach((file) => {
            expect(
                rootDirAllowedFiles.indexOf(file),
                `File "${file}" should not be in root or added to predifined list`
            ).not.toBe(-1)
        })
    })
})
...
describe(`Test "blockchains" folder`, () => {
    ...
    test(`Chain should have "logo.png" image`, () => {
        foundChains.forEach((chain) => {
            ...
            expect(
                isPathExistsSync(chainLogoPath),
                `File missing at path "${chainLogoPath}"`
            ).toBe(true)
    ...
    test('Chain folder must have lowercase naming', () => {
        foundChains.forEach((chain) => {
            expect(
                isLowerCase(chain),
                `Chain folder must be in lowercase "${chain}"`
            ).toBe(true)
        })
    })
                ...
                test(`Test asset folder allowed files on chain: ${chain}`, () => {
                    readDirSync(assetsPath).forEach((address) => {
                        ...
                        readDirSync(assetFiles).forEach((assetFolderFile) => {
                            expect(
                                assetFolderAllowedFiles.indexOf(
                                    assetFolderFile
                                ),
                                `File "${assetFolderFile}" not allowed at this path: ${assetsPath}`
                            ).not.toBe(-1)
                        })
                    })
                })
        ...
        ethSidechains.forEach((chain) => {
            test(`Test chain ${chain} folder`, () => {
                getChainAssetsList(chain).forEach((address) => {
                    ...
                    expect(
                        isPathDir(assetPath),
                        `Expect directory at path: ${assetPath}`
                    ).toBe(true)

                    const checksum = isChecksum(address)
                    expect(
                        checksum,
                        `Expect asset at path ${assetPath} in checksum - ${address}`
                    ).toBe(true)
                    ...
                    if (lowercase) {
                        expect(
                            checksum,
                            `Lowercase address ${address} on chain ${chain} should be in checksum`
                        ).toBe(true)
                    }

                    const assetLogoPath = getChainAssetLogoPath(chain, address)
                    expect(
                        isPathExistsSync(assetLogoPath),
                        `Missing file at path "${assetLogoPath}"`
                    ).toBe(true)

                    const [isDimentionOK, dimensionMsg] = isLogoDimentionOK(
                        assetLogoPath
                    )
    ...
    describe(`Check "binace" folder`, () => {
        it('Asset must exist on chain and', async () => {
            ...
            assets.forEach((asset) => {
                expect(
                    tokenSymbols.indexOf(asset),
                    `Asset ${asset} missing on chain`
                ).not.toBe(-1)
            })
        })
    })
    ...
    describe(`Check "tron" folder`, () => {
        ...
        test('Expect asset to be TRC10 or TRC20', () => {
            readDirSync(path).forEach((asset) => {
                expect(
                    isTRC10(asset) || isTRC20(asset),
                    `Asset ${asset} non TRC10 nor TRC20`
                ).toBe(true)

                const assetsLogoPath = getChainAssetLogoPath(Tron, asset)
                expect(
                    isPathExistsSync(assetsLogoPath),
                    `Missing file at path "${assetsLogoPath}"`
                ).toBe(true)
        ...
        stakingChains.forEach((chain) => {
            ...
            test(`Chain ${chain} validator must have correct structure and valid JSON format`, () => {
                validatorsList.forEach((val: ValidatorModel) => {
                    expect(
                        isValidatorHasAllKeys(val),
                        `Some key and/or type missing for validator ${JSON.stringify(
                            val
                        )}`
                    ).toBe(true)
                    expect(
                        isValidJSON(validatorsListPath),
                        `Not valid json file at path ${validatorsListPath}`
                    ).toBe(true)
                })
            })
            ...
            test(`Chain ${chain} validator must have corresponding asset logo`, () => {
                validatorsList.forEach(({ id }) => {
                    ...
                    expect(
                        isPathExistsSync(path),
                        `Chain ${chain} asset ${id} logo must be present at path ${path}`
                    ).toBe(true)
            ...
            test('Make sure validator has corresponding logo', () => {
                validatorsList.forEach((val) => {
                    expect(
                        chainValidatorsAssetsList.indexOf(val.id),
                        `Expecting image asset for validator ${val.id} on chain ${chain}`
            ...
            test('Make sure validator asset logo has corresponding info', () => {
                chainValidatorsAssetsList.forEach((valAssetLogoID) => {
                    expect(
                        validatorsList.filter((v) => v.id === valAssetLogoID)
                            .length,
                        `Expect validator logo ${valAssetLogoID} to have info`
                    ).toBe(1)
                })
            })
        })
...
function testTezosValidatorsAssets(assets: string[]) {
    test('Tezos assets must be correctly formated tz1 address', () => {
        assets.forEach((addr) => {
            expect(
                eztz.crypto.checkAddress(addr),
                `Ivalid Tezos address: ${addr}`
            ).toBe(true)
        })
    })
}
...
function testWavesValidatorsAssets(assets: string[]) {
    test('WAVES assets must have correct format', () => {
        assets.forEach((addr) => {
            expect(
                isWavesAddress(addr),
                `Address ${addr} should be WAVES formated`
            ).toBe(true)
        })
    })
}
...
function testCosmosValidatorsAddress(assets: string[]) {
    test('Cosmos assets must have correct format', () => {
        assets.forEach((addr) => {
            expect(
                addr.startsWith('cosmosvaloper1'),
                `Address ${addr} should start from "cosmosvaloper1"`
            ).toBe(true)
            expect(addr.length, `Address ${addr} should have length 52`).toBe(
                52
            )
            expect(
                isLowerCase(addr),
                `Address ${addr} should be in lowercase`
            ).toBe(true)
        })
    })
}
...
function testKavaValidatorsAddress(assets: string[]) {
    test('Kava assets must have correct format', () => {
        assets.forEach((addr) => {
            expect(
                addr.startsWith('kavavaloper1'),
                `Address ${addr} should start from "kavavaloper1"`
            ).toBe(true)
            expect(addr.length, `Address ${addr} should have length 50`).toBe(
                50
            )
            expect(
                isLowerCase(addr),
...
function testTerraValidatorsAddress(assets: string[]) {
    test('Terra assets must have correct format', () => {
        assets.forEach((addr) => {
            expect(
                addr.startsWith('terravaloper1'),
                `Address ${addr} should start from "terravaloper1"`
            ).toBe(true)
            expect(addr.length, `Address ${addr} should have length 51`).toBe(
                51
            )
            expect(
                isLowerCase(addr),
...
describe('Test Coinmarketcap mapping', () => {
    const cmcMap = JSON.parse(
        readFileSync('./pricing/coinmarketcap/mapping.json')
    )
    ...
    test(`Items must be sorted by "id" in ascending order`, () => {
        cmcMap.forEach((el, i) => {
            if (i > 0) {
                ...
                expect(
                    curID,
                    `Item ${curID} must be greather or equal to ${prevID}`
            ...
            if (i > 0) {
                ...
                if (prevID == curID) {
                    expect(
                        ...
                        `Item ${JSON.stringify(
                            el
                        )} must be greather or equal to ${JSON.stringify(
                            prevEl
                        )}`
        ...
        cmcMap.forEach((el) => {
            Object.keys(el).forEach((key) => {
                ...
                if (typeof val === 'string') {
                    expect(
                        val.indexOf(' ') >= 0,
                        ` Property value "${val}" should not contain space`
                    ).toBe(false)
                }
            })
        })
    ...
    test('Params should have value and correct type', () => {
        cmcMap.forEach((el) => {
            ...
            expect(typeof coin, `Coin ${coin} must be type "number"`).toBe(
                'number'
            )

            expect(
                ['token', 'coin'],
                `Element with id ${id} has wrong type: "${type}"`
            ).toContain(type)
            if (type === 'token') {
                expect(
                    token_id,
                    `token_id ${token_id} with id ${id} must be type not empty`
                ).toBeTruthy()
            }
            ...
            if (type === 'coin') {
                expect(
                    el,
                    `Element with id ${id} should not have property "token_id"`
                ).not.toHaveProperty('token_id')
            }
        })
    })
        ...
        cmcMap.forEach((el) => {
            ...
            switch (coin) {
                case 60 && type === TickerType.Token:
                    expect(
                        isChecksum(token_id),
                        `"token_id" ${token_id} with id ${id} must be in checksum`
                    ).toBe(true)
                    break
                case 195 && type === TickerType.Token:
                    expect(
                        isTRC10(token_id) || isTRC20(token_id),
                        `"token_id" ${token_id} with id ${id} must be in TRC10 or TRC20`
                    ).toBe(true)
                    break
                case 714 && type === TickerType.Token:
                    expect(
                        tokenSymbols.indexOf(token_id),
                        `"token_id" ${token_id} with id ${id} must be BEP2 symbol`
                    ).toBe(true)
                default:
                    break
            }
...
describe('Test blacklist and whitelist', () => {
    const assetsChains = readDirSync(chainsFolderPath).filter((chain) =>
        isPathExistsSync(getChainAssetsPath(chain))
    )
    ...
    assetsChains.forEach((chain) => {
        ...
        test(`Whitelist should not contain assets from blacklist on ${chain} chain`, () => {
            ...
            whiteList.forEach((a) => {
                expect(
                    a in blacklistMap,
                    `Found whitelist asset ${a} in blacklist on chain ${chain}`
                ).toBe(false)
            })
        })
        ...
        test(`Blacklist should not contain assets from whitelist on ${chain} chain`, () => {
            ...
            blackList.forEach((a) => {
                expect(
                    a in whitelistMap,
                    `Found blacklist asset ${a} in whitelist on chain ${chain}`
                ).toBe(false)
            })
        })
    })
...
describe('Test all JSON files to have valid content', () => {
    ...
    files.forEach((file) => {
        expect(isValidJSON(file), `${file} path contains invalid JSON`).toBe(
            true
        )
    })
})
...
describe('Test helper functions', () => {
    test(`Test getHandle`, () => {
        ...
        urls.forEach((u) => {
            expect(getHandle(u.url), `Getting handle from url ${u}`).toBe(
                u.expected
            )
        })
    })
